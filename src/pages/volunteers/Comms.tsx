        ...
        <div className="px-5 py-4 space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Orientation Confirmation"
            />
          </div>
          <div>
          ...
          <tbody>
            {logs.map((log, i) => (
              <tr key={log.id} className={`border-b border-gray-100 ${i % 2 === 0 ? "" : "bg-gray-50/50"}`}>
                ...
                <td className="px-3 py-3 text-center"><span className="text-blue-600 font-medium">{log.deliveredCount}</span></td>
                <td className="px-3 py-3 text-center"><span className="text-green-600 font-medium">{log.readCount}</span></td>
    ...
    <div className="flex h-full bg-white">
      ...
      <div className="w-72 border-r border-gray-200 flex flex-col shrink-0">
        <div className="px-4 py-4 border-b border-gray-200">
          <h2 className="font-semibold text-gray-900 text-sm mb-3">Volunteer Comms</h2>
      ...
      {showProfile && activeContact && (() => {
        const volunteer = mockVolunteers.find(v => v.id === activeContactId)
        return volunteer ? (
          <VolunteerPane volunteer={volunteer} onClose={() => setShowProfile(false)} />
        ) : null
      })()}
    </div>
  )
