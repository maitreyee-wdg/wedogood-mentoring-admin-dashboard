...
import {
  mockVolunteerContacts, mockConversations, commsTemplates,
  type CommContact, type ChatMessage,
} from "@/data/commsData"
import { Search, Send, ChevronDown, Check, CheckCheck, MessageSquare } from "lucide-react"

export default function VolunteersComms() {
  const [contacts] = useState<CommContact[]>(mockVolunteerContacts)
    ...
    <div className="flex h-full bg-white">
      ...
      <div className="w-72 border-r border-gray-200 flex flex-col shrink-0">
        <div className="px-4 py-4 border-b border-gray-200">
          <h2 className="font-semibold text-gray-900 text-sm mb-3">WhatsApp Comms</h2>
          <div className="relative">
            <Search className="absolute left-3 top-2.5 h-4 w-4 text-gray-400" />
            <Input placeholder="Search volunteers…" className="pl-9 text-sm" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {filteredContacts.map((c) => (
            ...
            >
              ...
              {c.unread > 0 && (
                <span className="mt-1 w-4 h-4 bg-green-500 text-white text-xs rounded-full flex items-center justify-center shrink-0">
                  {c.unread}
                </span>
              )}
            </button>
        ...
        <div className="flex-1 flex flex-col min-w-0">
                  ...
                  <div className="flex border-b border-gray-200 px-1 pt-1">
                    {(["generic", "engagement"] as const).map((tab) => (
                      <button key={tab} onClick={() => setTemplateTab(tab)}
                        className={`flex-1 py-2 text-xs font-medium capitalize rounded-t-lg transition-colors ${templateTab === tab ? "bg-blue-50 text-blue-700" : "text-gray-500 hover:text-gray-700"}`}>
                        {tab === "generic" ? "Generic" : "Engagement"}
                      </button>
                    ))}
                  </div>
                  <div className="p-2 space-y-1 max-h-64 overflow-y-auto">
                    {(templateTab === "generic" ? genericTemplates : engagementTemplates).map((t) => (
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 bg-gray-50" onClick={() => setShowTemplates(false)}>
            {activeMessages.length === 0 ? (
              <div className="flex items-center justify-center h-full text-gray-400 text-sm">No messages yet. Use a template or type a message.</div>
